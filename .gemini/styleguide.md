...
## 0.2. 🧹 보이스카우트 원칙 (Boy Scout Rule)

보이스카우트 원칙은 **“코드를 발견했을 때보다 더 깨끗하게 만들어 놓고 떠나라”**라는 철학으로, 지속적으로 코드 품질을 향상시키기 위한 개발 문화 지침입니다.

## 0.2.1. 핵심 개념
PR을 생성하거나 코드를 수정할 때, 해당 부분이 자신이 작성한 코드가 아니더라도 다음을 준수합니다.
작은 개선이라도 누적하면 큰 품질 향상으로 이어진다.
난독성을 줄이는 방향으로 코드 구조를 조금이라도 개선한다.
중복 제거, 네이밍 개선, 불필요한 복잡성 제거 등을 적극 포함한다.
주석·미사용 변수·죽은 코드(dead code) 등을 발견하면 정리한다.
테스트 가능성·확장 가능성을 조금이라도 높이는 방향으로 개선한다.

## 0.2.2. PR 리뷰 기준

Gemini Code Assistant는 PR 리뷰 시 다음 사항을 확인하고 피드백해야 합니다.
기존 코드 개선이 가능한 부분을 무시하지 않았는가?
“내가 수정한 부분만”이 아니라, 주변 코드 품질도 함께 개선했는가?
코드가 더 읽기 쉽고 더 간단해졌는가?
불필요한 코드나 혼란을 유발하는 부분이 남아 있지 않은가?

작은 한 줄의 정리라도 프로젝트 전체 품질에 큰 영향을 주므로, 모든 PR은 보이스카우트 정신을 바탕으로 작성되어야 합니다.

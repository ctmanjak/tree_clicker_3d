...
public class UpgradeButtonAnimator : MonoBehaviour
{
    ...
    public void PlayPurchaseAnimation()
    {
        ...
        if (_backgroundImages != null && _originalColors != null)
        {
            for (int i = 0; i < _backgroundImages.Length; i++)
            {
                if (_backgroundImages[i] != null)
                {
                    int index = i;
                    _currentSequence.Join(_backgroundImages[index].DOColor(_purchaseFlashColor, _purchaseDuration * 0.3f));
                }
            }
            _currentSequence.AppendCallback(() =>
            {
                for (int i = 0; i < _backgroundImages.Length; i++)
                {
                    if (_backgroundImages[i] != null)
                    {
                        _backgroundImages[i].DOColor(_originalColors[i], _purchaseDuration * 0.7f);
                    }
                }
            });
        }
    ...
    public void PlayDeniedAnimation()
    {
        ...
        if (_backgroundImages != null && _originalColors != null)
        {
            for (int i = 0; i < _backgroundImages.Length; i++)
            {
                ...
                {
                    int index = i;
                    _currentSequence.Join(_backgroundImages[index].DOColor(_deniedFlashColor, _shakeDuration * 0.3f));
                }
            }
            _currentSequence.AppendCallback(() =>
            {
                for (int i = 0; i < _backgroundImages.Length; i++)
                {
                    if (_backgroundImages[i] != null)
                    {
                        _backgroundImages[i].DOColor(_originalColors[i], _shakeDuration * 0.7f);
                    }
                }
            });
        }

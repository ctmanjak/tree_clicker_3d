...
- [ ] InputHandler.cs 컴파일 성공
- [ ] GameManager.cs 컴파일 성공
- [ ] GameManager 싱글톤 정상 동작 (중복 생성 시 파괴)
- [ ] ServiceLocator.Get<GameManager>() 정상 동작
- [ ] 마우스 클릭 시 콘솔에 클릭 위치 출력 (디버그용)
- [ ] AddWood() 호출 시 OnWoodChanged 이벤트 발행
